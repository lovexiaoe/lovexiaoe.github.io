...
```
#### --no-off 参数。
gitlab的MergeRequest和github的PullRequest都是一种分支合并请求，和其他的合并分支一样。
但是在merge的时候，通常会带上--no-off参数，no-off参数会保留之前的分支历史。而不带这个参数，只会保留单条分支记录。
...
提交代码前,我们不知道远程库的代码是否有变动，所以要先pull拉远程库的代码下来。
```
